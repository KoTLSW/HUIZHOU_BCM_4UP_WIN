...
PresentIndex:(int)presentIndex TotalIndex:(int)totalIndex
{
    ...
    switch (sfcCheckType)
    {
        ...
        case e_LOG_CHECK:
        {
            ...
            {
                [urlString appendFormat:@"&p%d=%@",i+3,array[i]];
            }
...
- (BOOL) Option:(NSString*)option checkSerialNumber:(NSString *)sn testResult:(NSString*)testResult startTime:(NSString*)startTime testArgument:(NSArray *)array PresentIndex:(int)presentIndex TotalIndex:(int)totalIndex
{
    ...
    if ([option isEqualToString:@"isConnectServer"]) {
        url = [self createURL:e_SEVER_CHECK sn:nil testResult:nil
                    startTime:startTime testArgument:nil endTime:nil bdaSerial:nil faiureMessage:nil PresentIndex:presentIndex TotalIndex:totalIndex];
    }
    ...
    else
    {
        url = [self createURL:e_LOG_CHECK sn:sn testResult:testResult
                    startTime:startTime testArgument:array endTime:nil bdaSerial:nil faiureMessage:nil PresentIndex:presentIndex TotalIndex:totalIndex];
    }
