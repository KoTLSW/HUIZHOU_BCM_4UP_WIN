...
//SFC检测上传sn
-(BOOL)StepSFC_CheckUploadSN:(BOOL)isUploadSFC Option:(NSString*)option testResult:(NSString*)testResult startTime:(NSString*)startTime testArgument:(NSArray*)array PresentIndex:(int)presentIndex TotalIndex:(int)totalIndex
{
    [_lock lock];
    ...
    if(isUploadSFC)
    {
        ...
        if ([option isEqualToString:@"isPassOrNot"]) {
            ...
            [[BYDSFCManager Instance] Option:option checkSerialNumber:_strSN testResult:nil startTime:startTime testArgument:nil PresentIndex:presentIndex TotalIndex:totalIndex];
        }
        else if ([option isEqualToString:@"isConnectServer"]){
            [[BYDSFCManager Instance] setSFCCheckType:e_SEVER_CHECK];
            [[BYDSFCManager Instance] Option:option checkSerialNumber:nil testResult:nil startTime:startTime testArgument:nil PresentIndex:presentIndex TotalIndex:totalIndex];
        }else{
            [[BYDSFCManager Instance] setSFCCheckType:e_LOG_CHECK];
            [[BYDSFCManager Instance] Option:option checkSerialNumber:_strSN testResult:testResult startTime:startTime testArgument:array PresentIndex:presentIndex TotalIndex:totalIndex];
        }
